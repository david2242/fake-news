...
export class UserDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userForm.patchValue({
      ...
      userImage: this.currentUser.imageRef,
    });
  }
  ...
  private saveUserDataFromInput(): void {
    ...
    if (this.userForm.value.userImage) this.currentUser.imageRef = this.userForm.value.userImage;
  }
    ...
    if (this.imageFile) {
      ...
      this.httpClient.post(this.CLOUDINARY_URL, fd).subscribe(
        res => {
          ...
          this.currentUser.imageRef = this.responseObj.secure_url;
